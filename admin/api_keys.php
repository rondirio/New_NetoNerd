...
function copiarChave(chave) {
    const mostrarToast = () => {
        let toast = document.getElementById('nn-copy-toast');
        if (!toast) {
            toast = document.createElement('div');
            toast.id = 'nn-copy-toast';
            toast.style.cssText = 'position:fixed;bottom:24px;right:24px;background:#28a745;color:#fff;padding:12px 20px;border-radius:6px;font-size:14px;z-index:9999;box-shadow:0 4px 12px rgba(0,0,0,.2);transition:opacity .3s';
            toast.innerHTML = '<i class="fas fa-check"></i> Chave copiada!';
    ...
    navigator.clipboard.writeText(chave).then(mostrarToast).catch(() => {
